...
def compute_SDF(working_q, output_q):
    ...
    feature_gen = DFG(method='LARS',
                      ...
                      version=1,
                      find_alpha=False,
                      alpha=[1e-8],# [0.00015, 0.00017, 0.00010, 0.00015, 0.00009, 0.00012, 0.00016, 0.00020, 0.00023, 0.00031, 0.00033, 0.00026],
                      model_freq=np.linspace(0.5, 45, 25, endpoint=True),
                      normalize=True,
    ...
    while True:
        ...
        if temp is not None and not isinstance(temp, str):
            ...
            # print(np.sum([*map(np.count_nonzero, features)]) + np.sum([*map(np.count_nonzero, features)]),',')
...
if __name__ == '__main__':
    ...
    for x, meta_data in reader.read(condition=conditions, random=random):
        ...
        working_q = utils.init_working_q(x[:, :, 0], exams_id, cpu_count)
        output_q = Queue()
...
alpha for np.linspace(3, 45, 23, endpoint=True) 
    (-0.2, 0.4)
        [0.00013, 0.00014, 0.00009, 0.00013, 0.00009, 0.00010, 0.00015, 0.00019, 0.00022, 0.00028, 0.00028, 0.00022  ],
    (-0.2, 0.1) 
