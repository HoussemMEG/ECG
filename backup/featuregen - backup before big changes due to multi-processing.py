...
class DFG:
    ...
    def generate(self, y: np.ndarray) -> Tuple[Optional[List[np.ndarray]], Optional[List[np.ndarray]]]:
        ...
        if self._method in ['LARS', 'LASSO']:
            ...
            #
            # for target_idx in range(self._n_target):
            #     p = multiprocessing.Process(target=self._lasso, args=(lasso, y[:, [target_idx]]))
            #     print('process {:} started'.format(target_idx))
            #     p.start()
            #     processes.append(p)
            #
            # for i, p in enumerate(processes):
            #     p.join()
            #     print('process {:} ended'.format(i))
    ...
    def _lasso(self, lasso, y: np.ndarray):
        ...
        with warnings.catch_warnings(record=True) as warning:
            ...
            alpha_path, active, coef_path = linear_model.lars_path(self._phi, y[:, 0], method='lasso', alpha_min=self.alpha, max_iter=self._max_iter)
    ...
    def _check(self):
        ...
        if self._version == 1 and not self._normalize and self._merging_weight != 0.5:
    ...
    def _select(self):
        ...
        if self._idx_selection:  # if I specify alpha lower value selection
            for target_idx in range(self._n_target):
            ...
            if all(isinstance(x, int) for x in self._selection) and not self._fast:
                for target_idx in range(self._n_target):
                    n_path = self.coef_path[target_idx].shape[1]
                    selection = sorted(list(set([x for x in self._selection if x < n_path])))
                    self.coef_path[target_idx] = self.coef_path[target_idx][:, selection]
                    self.alpha_path[target_idx] = self.alpha_path[target_idx][selection]
                    self.intercept_path[target_idx] = self.intercept_path[target_idx][selection]
